...
pub(crate) fn ui_main_system(
    ...
    if let Ok((camera, camera_transform)) = camera.single() {
        ...
        active
            .single()
            .map(|(_, timer, t)| {
                if let Some(pos) = camera.world_to_screen(&windows, camera_transform, t.translation)
                {
                    let margin = 64f32;
                    let pos = egui::Vec2::new(
                        pos.x - margin,
                        windows.get_primary().map(|x| x.height()).unwrap_or(0f32) - pos.y - margin,
                    );
                    egui::containers::Area::new("timer")
                        ...
                        .show(ctx.ctx(), |ui| {
                            let (_, paint) =
                                ui.allocate_painter(egui::Vec2::new(32f32, 32f32), Sense::click());
